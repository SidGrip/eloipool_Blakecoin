...
try:
	...
	if os.path.exists(_srcdir + '/.I_swear_that_I_am_Luke_Dashjr'):
		...
		_SourceFiles = subprocess.check_output(
			['git', '-C', _srcdir, 'ls-files'],
			text=True,
		...
		_GitDesc = subprocess.check_output(
			['git', '-C', _srcdir, 'describe', '--dirty', '--always'],
			text=True,
			stderr=subprocess.STDOUT,
		).strip().encode('utf8')
except BaseException as e:
	logging.getLogger('Licensing').warning(
		'Unable to enumerate source files from git; disabling direct source download support for this runtime.\n'
