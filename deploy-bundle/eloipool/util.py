...
def target2bdiff(target):
	# Rejected/high-hash share paths do not always carry a populated target.
	# Treat missing/zero targets as no-difficulty data instead of crashing the
	# share logger while formatting an otherwise useful rejection record.
	if not target:
		return _maybe_int(0)
